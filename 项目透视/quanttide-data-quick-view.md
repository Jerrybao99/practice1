...
量潮数据工程（quanttide-data）是量潮"第二大脑"知识管理体系中的数据工程域仓库和 **聚合入口仓库**。它 **本身不含代码**，通过 Git 子模块将 26 个独立仓库聚合为一个单体仓库，覆盖程序性记忆（文档/规则）、陈述性记忆（数据/记录）、可执行应用、实验项目和共享工具包，为数据工程团队提供统一的版本化管理入口。
...
- 工作意图应紧扣最终目标，即描绘量潮为什么想要建设这样一个数据工程，第二大脑
- 工作意图是“未来的自我记忆”，关系到组织希望成为一个什么样的组织，一般相对稳定，但和组织高度绑定，不一定具备很强的迁移性
- 工作洞察是“未来的语义记忆”，最大特点是可以从一个领域迁移到另外一个领域，也可以从一个组织迁移到另外一个组织，这是形成未来知识体系的核心出发点
...
| `docs/specification/index.md` | 数据工程格式标准入口 | 理解工程规范时 |
| `docs/bylaw/index.md` | 数据工程治理章程 | 了解流程约束时 |
| `docs/handbook/index.md` | JupyterBook 全生命周期手册 | 系统学习数据工程时 |
| `data/context/design-philosophy.md` | 7 条设计原则 | 理解团队设计哲学时 |
| `data/profile/ghtorrent/` | 蓝图三格式完整案例 | 学习 Blueprint 模式时 |
...
**子模块聚合管理分层策略：**本项目本身不含任何代码或文档内容，其核心功能是通过 `.gitmodules` 将 24 个独立 Git 仓库注册为子模块，实现集中索引、统一版本追踪和批量操作。
...
**DevOps 自动化：**通过 `qtcloud-devops` CLI 工具覆盖完整开发生命周期：
...
**分层提交原则：**子模块变更遵循"两层提交"规则，防止直接修改子模块文件导致引用混乱：
